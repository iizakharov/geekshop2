...
class Command(BaseCommand):
    def handle(self, *args, **options):
        # test_products = Product.objects.\
        #     filter(Q(category__name='офис') | Q(category__name='модерн')).\
        #     select_related()
        #
        # # test_products = Product.objects.exclude(category__name='офис', category__pk=1).select_related()
        # # test_products = Product.objects.exclude(category__name='офис').exclude(category__pk=1).select_related()
        #
        ...
        for orderitem in test_orderss:
            print(f'{orderitem.action_order:2}: заказ №{orderitem.pk:3}:\
                   ...
                   {abs(orderitem.total_discount):6.2f} руб. | \
